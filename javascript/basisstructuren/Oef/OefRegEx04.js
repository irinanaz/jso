...
function controleerInvoer(){
    var rrn;
    var exp= new RegExp("^\\d{6}.*\\d{3}.*\\d{2}$");
    var expDatum = RegExp("^\\d{2}((0[1-9])|(1[0-2]))((0[1-9])|(1[0-9])|(2[0-9])|(^3[0-1]))","g");
    var juist =false;
    rrn = toetsenbord.question("rijksregisternummer: ");
    
    while (exp.test(rrn)==false || expDatum.test(rrn)== false){
        console.log(exp.test(rrn));
        console.log(expDatum.test(rrn));
        rrn = toetsenbord.question("Geef de juiste rijksregisternummer: ");
...
var rijksregisterNummer = controleerInvoer();
